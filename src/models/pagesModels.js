...
module.exports = class PageModel {
  ...
  static async insertPages(page) {
    const { page_tittle, page_status, page_content, position_positon_id } =
      page;
    const insertPages = `INSERT INTO pages (page_title, page_status, page_content, position_id) VALUES (?,?,?,?);`;
  ...
  static async updatePage(page_id, page) {
    const { page_title, page_status, page_content, position_positon_id } = page;
    const insertPages = `UPDATE pages SET page_title = ?, page_status = ?, page_content = ?, WHERE page_id = ?;`;
    const [result] = await database.query(insertPages, [
      page_title,
      page_status,
      page_content,
